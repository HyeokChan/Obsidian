...
2. 재해복구 vs 고가용성
- 재해복구
	- 전체 워크로드를 별도의 장소에 복사하는 것에 중심
	- 재해복구의 목표는 비즈니스의 연속성
- 고가용성
	- 일부 워크로드의 서비스를 지속하는 것에 중심
	- 재해복구보다 구체적이고 작은 범위
	- 고가용성의 목표는 시스템의 기능 수행 시간의 최대화
3. Recovery Objectives
- RPO : 복구시점목표, 손실을 감당할 수 있는 데이터의 양
- RTO : 복구시간목표, 프로세스 복구하는데 걸리는 목표 시간
4. 재해복구 전략
- Backup & Restore : 백업 및 복구
- Pilot Light : 코어 시스템만 복제 후 대기
- Warm standby : 전체 시스템을 작은 스케일로 구성하여 대기
- Multi-site : 전체시스템을 동일하게 복제하여 동시 운영
- 뒤로 갈수록 금액은 비싸지만 복구 시간이 짧음
